<script lang="ts">
	...
	type selectedPriceType = '0-100' | '101-300' | '301-600' | '601-1000' | 'over 1001';
	let selectedPrice: string | selectedPriceType | '' = $page.url.searchParams.get('price') || 'all';
	const priceFilterList = ['0-100', '101-300', '301-600', '601-1000', 'over 1001'];
	let selectedRating: number = $page.url.searchParams.get('rating')
		? Number($page.url.searchParams.get('rating'))
		: 1;
...
<div class="flex flex-col items-start justify-start w-full h-full min-h-full gap-8 min-w-screen">
	...
	<main
		class="flex flex-row items-start justify-start w-full h-full max-w-full min-h-full gap-6 mainSection"
	>
			...
			<div class="flex flex-col gap-3 filtersList">
				...
				{#if showCategoryFilter}
					<div class="flex flex-col w-full">
						{#each ['all', ...data.categories.map((cat) => cat.name)] as category}
							<button
								aria-label="Filter"
		...
		<div
			class="items-stretch justify-between flex-1 w-full h-full max-w-full min-h-full pl-6 border border-r-0 border-zinc-800 border-y-0"
		>
			{#await data.products}
				<p>Loading...</p>
			{:then products}
