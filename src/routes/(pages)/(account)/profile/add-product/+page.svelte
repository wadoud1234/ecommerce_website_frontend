...
<div class="flex flex-col items-start justify-start w-full h-full max-w-full max-h-full min-w-full min-h-full gap-4">
    <h1 class="text-3xl font-medium ">Add Product</h1>

    <form  method="POST" class="flex flex-row items-start justify-start w-full h-full gap-10">
        <!-- LEFT -->
        <div class="lg:max-w-[50%] flex flex-col items-start justify-start flex-1 h-full min-h-full gap-2">
            {#if $message}  
                <p>{$message}</p>
            {/if}
            <Form.Field {form} name="name" class="w-full">
                <Form.Control let:attrs>
                    ...
                    <Input
                        ...
                        max="255"
                        required
                        class="border bg-inherit border-zinc-400 dark:border-zinc-800 border-inherit"
                        {...attrs}
                        type="text"
            ...
            <Form.Field {form} name="description" class="w-full">
                <Form.Control let:attrs>
                    ...
                    <Textarea
                        rows={5}
                        class="border resize-none bg-inherit border-zinc-400 dark:border-zinc-800 border-inherit"
                        {...attrs}
                ...
                <Form.Field {form} name="category" class="w-full">
                    <Form.Control let:attrs>
                        ...
                        <Select.Root
                            ...
                            >
                            <Select.Trigger {...attrs} class="bg-inherit hover:bg-zinc-200 dark:hover:bg-zinc-800 border-zinc-800">
        ...
        <!-- RIGHT -->
        <div class="md:m-0 md:max-w-[50%] w-full h-full flex items-start justify-center" >
            ...
            {:else}
                <div class="flex flex-col items-center justify-start w-full gap-2">
                    ...
                    {/if}
                    <Button class="gap-2 border rounded-md w-fit bg-zinc-200 dark:border-zinc-800 hover:bg-zinc-300 text-inherit border-zinc-400 dark:bg-zinc-800 dark:bg-inherit dark:hover:bg-zinc-800 dark:text-zinc-300">
                        <ImageUp/>
                        <CldUploadButton 
            ...
            {/if} -->
            <div class="flex flex-col items-center justify-start w-full h-full max-w-full min-w-full gap-y-4 gap-x-10">
                <AddMainProductImage 
                    uploadPreset="ecommerce_products" 
